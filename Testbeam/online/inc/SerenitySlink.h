...
namespace Hgcal10gLinkReceiver {
  ...
  public:
	...
	for(unsigned c(0);c<2;c++) {
	  ...
	  mc["spy.regs.cfg.user_pkt_len" ]=uhalSpyRead(c,"regs.cfg.user_pkt_len");
	  mc["spy.regs.cfg.user_l1a_diff"]=uhalSpyRead(c,"regs.cfg.user_l1a_diff");
    ...
    uint32_t uhalSpyRead(unsigned c, const std::string &s) {
      if(c==0) return uhalTopRead("payload.slink_spy0."+s);
      if(c==1) return uhalTopRead("payload.slink_spy1."+s);
      return 0;
    }
    
    bool uhalSpyWrite(unsigned c, const std::string &s, uint32_t v) {
      if(c==0) return uhalTopWrite("payload.slink_spy0."+s,v);
      if(c==1) return uhalTopWrite("payload.slink_spy1."+s,v);
      return false;
    }
