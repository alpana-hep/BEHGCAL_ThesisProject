...
f_pf = ROOT.TFile('hitData_maskingstudy_masking_partialfix_newcode.root')
f_nm = ROOT.TFile('hitData_maskingstudy_masking_partialfix_newcode_sec_p3.root')
...
for il in xrange(Nlayers):
    ...
    plot = plotlayer.MakeLayerCanvas('partial_rates_%.2i' % ilG, -6, 12, -1, 17, [min_z, max_z])
    plot['cmssw'] = {}
    ...
    for UV, module in mod_dict.iteritems():
        ...
        key = '%i_%i_%i_%i' % (ilG, module['type'], UV[0], UV[1])
        v_pf = f_pf.Get('trans_hits_%s' % key).GetMean()
        v_nm = f_nm.Get('trans_hits_%s' % key).GetMean()
        print v_pf, v_nm
        if v_pf != v_nm and v_pf > 0.:
            value = v_nm / v_pf
        else:
            value = -1
        # Compute the colour
