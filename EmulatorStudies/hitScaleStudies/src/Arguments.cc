...
void Arguments::setDefaults() {
  js_ = {
    ...
    {"econdHitAdditiveArg",EcondAsicConfiguration::defaultHitAdditiveArg_},
    {"econdHitlayerArg",EcondAsicConfiguration::defaultHitlayerArg_},
    {"econdScintHitScalingArg", EcondAsicConfiguration::defaultHitScalingArg_},
...
unsigned Arguments::econdScintHitScalingArg() const {
  return js_.at("econdScintHitScalingArg").get<unsigned>();
}

unsigned Arguments::econdHitAdditiveArg() const {
  //std::cout<<"alpana-js"<<"\t"<<js_.at("econdHitAdditiveArg")<<std::endl;
  return js_.at("econdHitAdditiveArg").get<unsigned>();
}
unsigned Arguments::econdHitlayerArg() const {
  return js_.at("econdHitlayerArg").get<unsigned>();
}
...
void Arguments::print() const {
  ...
  if(econdHitAdditiveArg()>0) std::cout << " ECON-D hit scaling or addition ";
  else std::cout << " ECON-D hit scaling or additive not set, defaulting to ";
  std::cout << econdHitAdditiveArg() << std::endl;
  if(econdHitlayerArg()>0) std::cout << " Which layer's ECON-D should be scaled -";
  else std::cout << "Layer is not assigned for scaling the hits,  defaulting to ";
  std::cout << econdHitAdditiveArg() << std::endl;
