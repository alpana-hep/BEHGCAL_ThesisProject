...
for dirname, index_on, label in scans:
    files = glob('%s/HGCalBufferModel_*.root' %basedir) #files = glob('%s/%s/HGCalBufferModel_*.root' % (basedir, dirname))
    proc_files = glob('%s/HGCalBufferModel_processed_*.root' %basedir1)#proc_files = glob('%s/%s/HGCalBufferModel_*_proc.root' % (basedir, dirname))
    dat[dirname] = dict()
...
leg_label % x_val                print ('Graph for ' + dirname)
                drawlist[-1].Print()
            drawlist[-1]._dirname = dirname
