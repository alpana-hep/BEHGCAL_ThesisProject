...
# "econdHitScaling_default_V10",
 # "econdHitScaling_default_V10_pseudo",
    "econdHitScaling_Additive_fracNcells_V10", ## difference case with different fractions (n10,n20,n30)
    "econdHitScaling_Additive_fracNcells_V10_pseudo",
    # "econdHitScaling_LDM_Additive_fracNcells_V10",
    # "econdHitScaling_LDM_Additive_fracNcells_V10_pseudo",
    # "LayerScaling_Additive_fracNcells_V10",
    # "LayerScaling_Additive_fracNcells_V10_pseudo",
    "econdHitScaling_default_V10_regTrig",
    "econdHitScaling_default_V10_regTrig_pseudo",
    "econdHitScaling_Additive_fracNcells_V10_regTrig",
...
if 'econdHitScaling_Additive_fracNcells_V10_pseudo' in do_submissions:
    resdir = '/home/work/kalpana1/public/EmulatorStudies/CMSSW_10_2_20/src/Output_ThrottlingVsTruncation/PseudoEvents/econdHitScaling_Additive_fracNcells_V10_pseudo'
    os.system('mkdir -p %s' % resdir)
    scale_num=[1,2,3,4]
        ...
        for level in range(100, 1100, 100):
            for bufflevel in range(1850, 2600, 100):                
                for irng, startPoint in enumerate(startPoints):
                    ...
                    cmd = './bin/HGCalBufferModel data/current_default_V10_pseudo.json outputname=\\"%s/%s.root\\" triggerRate=750 numberOfBx=%i fileReadMode=1 randomSeed=%i econdHitScaling=%i econdHitAdditiveArg=%s econdHitScalingArg=true econdHighBufferLevel=%i econdMediumBufferLevel=%i | gzip -c > %s/%s.log.gz' % (writedir, basename, nBx, irng + 1, level, scale_num[i],bufflevel, bufflevel-400, writedir, basename)
                    cmd += '; cp %s/%s.* %s/' % (writedir, basename, resdir)
                    job_mgr.job_queue.append(cmd)

if 'econdHitScaling_Additive_fracNcells_V10' in do_submissions:
    resdir = '/home/work/kalpana1/public/EmulatorStudies/CMSSW_10_2_20/src/Output_ThrottlingVsTruncation/PseudoEvents/econdHitScaling_Additive_fracNcells_V10'
    os.system('mkdir -p %s' % resdir)
    scale_num=[1,2,3,4]
    for i in range(len(scale_num)):
        for level in range(100, 1100, 100):
...
if 'econdHitScaling_Additive_fracNcells_V10_regTrig_pseudo' in do_submissions:
    resdir = '/home/work/kalpana1/public/EmulatorStudies/CMSSW_10_2_20/src/Output_ThrottlingVsTruncation/PseudoEvents/econdHitScaling_Additive_fracNcells_V10_regTrig_pseudo'
    os.system('mkdir -p %s' % resdir)
    scale_num=[1,2,3,4]
    for i in range(len(scale_num)):
        for level in range(100, 1100, 100):
            for bufflevel in range(1850, 2600, 100):
                for irng, startPoint in enumerate(startPoints):
                    ...
                    basename = 'HGCalBufferModel_case%i_add_%i_buffer_%i_%i_job_%i' % (scale_num[i],level, bufflevel, bufflevel-400, irng + 1)
                    cmd = './bin/HGCalBufferModel data/current_default_V10_pseudo.json outputname=\\"%s/%s.root\\" triggerRate=%s numberOfBx=%i fileReadMode=1 randomSeed=%i econdHitScaling=%i econdHitAdditiveArg=%s econdHitScalingArg=true econdHighBufferLevel=%i econdMediumBufferLevel=%i tcdsHllhcFilename=\\"Flat\\" triggerMethod=2053 | gzip -c > %s/%s.log.gz' % (writedir, basename,actualrate, nBx, irng + 1, level, scale_num[i],bufflevel, bufflevel-400, writedir, basename)
                    cmd += '; cp %s/%s.* %s/' % (writedir, basename, resdir)
                    job_mgr.job_queue.append(cmd)

if 'econdHitScaling_Additive_fracNcells_V10_regTrig' in do_submissions:
    resdir = '/home/work/kalpana1/public/EmulatorStudies/CMSSW_10_2_20/src/Output_ThrottlingVsTruncation/PseudoEvents/econdHitScaling_Additive_fracNcells_V10_regTrig'
    os.system('mkdir -p %s' % resdir)
    scale_num=[1,2,3,4]
    for i in range(len(scale_num)):
        for level in range(100, 1100, 100):
...
if 'econdHitScaling_default_V10_regTrig_pseudo' in do_submissions:
    resdir = '/home/work/kalpana1/public/EmulatorStudies/CMSSW_10_2_20/src/Output_ThrottlingVsTruncation/PseudoEvents/econdHitScaling_default_V10_regTrig_pseudo'
    os.system('mkdir -p %s' % resdir)
    for level in range(1000, 1801, 100):
        for bufflevel in range(1850, 2600, 100):
            for irng, startPoint in enumerate(startPoints):
                ...
                cmd = './bin/HGCalBufferModel data/current_default_V10_pseudo.json outputname=\\"%s/%s.root\\" triggerRate=%s numberOfBx=%i fileReadMode=1 randomSeed=%i econdHitScaling=%i econdHitScalingArg=true econdHighBufferLevel=%i econdMediumBufferLevel=%i tcdsHllhcFilename=\\"Flat\\" triggerMethod=2053 | gzip -c > %s/%s.log.gz' % (writedir, basename,actualrate, nBx, irng + 1, level, bufflevel, bufflevel-400, writedir, basename)
                cmd += '; cp %s/%s.* %s/' % (writedir, basename, resdir)
                job_mgr.job_queue.append(cmd)
                
if 'econdHitScaling_default_V10_regTrig' in do_submissions:
    resdir = '/home/work/kalpana1/public/EmulatorStudies/CMSSW_10_2_20/src/Output_ThrottlingVsTruncation/PseudoEvents/econdHitScaling_default_V10_regTrig'
    os.system('mkdir -p %s' % resdir)
    for level in range(1000, 1801, 100):
        for bufflevel in range(1850, 2600, 100):
            for irng, startPoint in enumerate(startPoints):
...
#     resdir = '/home/work/kalpana1/public/EmulatorStudies/CMSSW_10_2_20/src/Output_ThrottlingVsTruncation/econdHitScaling_default_V10'
#     os.system('mkdir -p %s' % resdir)
...
if 'triggerRate_default_V10_pseudo' in do_submissions:
    resdir = '/home/work/kalpana1/public/EmulatorStudies/CMSSW_10_2_20/src/Output_ThrottlingVsTruncation/PseudoEvents/triggerRate_default_V10_pseudo'
    os.system('mkdir -p %s' % resdir)
    # Do scans of L1 rate
    for rate in range(600, 1000, 50):
        for bufflevel in range(1850, 2600, 100):
            for irng, startPoint in enumerate(startPoints):
                ...
                cmd = './bin/HGCalBufferModel data/current_default_V10_pseudo.json outputname=\\"%s/%s.root\\" numberOfBx=%i fileReadMode=1 randomSeed=%i triggerRate=%i econdHighBufferLevel=%i econdMediumBufferLevel=%i | gzip -c > %s/%s.log.gz' % (writedir, basename, nBx, irng + 1, rate, bufflevel, bufflevel-400,writedir, basename)
                cmd += '; cp %s/%s.* %s/' % (writedir, basename, resdir)
                job_mgr.job_queue.append(cmd)

if 'triggerRate_default_V10' in do_submissions:
    resdir = '/home/work/kalpana1/public/EmulatorStudies/CMSSW_10_2_20/src/Output_ThrottlingVsTruncation/PseudoEvents/triggerRate_default_V10'
    os.system('mkdir -p %s' % resdir)
    # Do scans of L1 rate                                                                                                                                            
    for rate in range(600, 1000, 50):
        for bufflevel in range(1850, 2600, 100):
...
if 'triggerRate_avEventPacket_V10_pseudo' in do_submissions:
    resdir = '/home/work/kalpana1/public/EmulatorStudies/CMSSW_10_2_20/src/Output_ThrottlingVsTruncation/PseudoEvents/triggerRate_avEventPacket_V10_pseudo'    
    os.system('mkdir -p %s' % resdir)
    for rate in range(600, 1000, 50):
        for bufflevel in range(1850, 2600, 100):
            for irng, startPoint in enumerate(startPoints):
                basename = 'HGCalBufferModel_%i_buffer_%i_%i_job_%i' % (rate, bufflevel, bufflevel-400,irng + 1)
                cmd = './bin/HGCalBufferModel data/current_default_V10_pseudo.json outputname=\\"%s/%s.root\\" numberOfBx=%i fileReadMode=4 randomSeed=%i triggerRate=%i econdHighBufferLevel=%i econdMediumBufferLevel=%i | gzip -c > %s/%s.log.gz' % (writedir, basename, nBx, irng + 1, rate, bufflevel, bufflevel-400,writedir, basename)
                cmd += '; cp %s/%s.* %s/' % (writedir, basename, resdir)
                job_mgr.job_queue.append(cmd)


if 'triggerRate_avEventPacket_V10' in do_submissions:
    resdir = '/home/work/kalpana1/public/EmulatorStudies/CMSSW_10_2_20/src/Output_ThrottlingVsTruncation/PseudoEvents/triggerRate_avEventPacket_V10'
    os.system('mkdir -p %s' % resdir)
    for rate in range(600, 1000, 50):
        for bufflevel in range(1850, 2600, 100):
            for irng, startPoint in enumerate(startPoints):
...
if 'triggerRate_regTrig_V10_pseudo' in do_submissions:
    resdir = '/home/work/kalpana1/public/EmulatorStudies/CMSSW_10_2_20/src/Output_ThrottlingVsTruncation/PseudoEvents/triggerRate_regTrig_V10_pseudo'
    os.system('mkdir -p %s' % resdir)
    # Do scans of L1 rate
    # If we want to factor in the number of colliding bunches = 2748 / 3564
...
if 'triggerRate_regTrig_V10' in do_submissions:
    resdir = '/home/work/kalpana1/public/EmulatorStudies/CMSSW_10_2_20/src/Output_ThrottlingVsTruncation/PseudoEvents/triggerRate_regTrig_V10'
    os.system('mkdir -p %s' % resdir)
    # Do scans of L1 rate                                                                                                                                             
    # If we want to factor in the number of colliding bunches = 2748 / 3564                                                                                           
    for rate in range(40, 66, 2):
...
if 'triggerRate_avEventPacket_regTrig_V10_pseudo' in do_submissions:
    resdir = '/home/work/kalpana1/public/EmulatorStudies/CMSSW_10_2_20/src/Output_ThrottlingVsTruncation/PseudoEvents/triggerRate_avEventPacket_regTrig_V10_pseudo'
    os.system('mkdir -p %s' % resdir)
    # Do scans of L1 rate
    for rate in range(40, 66, 2):
        for bufflevel in range(1850, 2600, 100):
...
if 'triggerRate_avEventPacket_regTrig_V10' in do_submissions:
    resdir = '/home/work/kalpana1/public/EmulatorStudies/CMSSW_10_2_20/src/Output_ThrottlingVsTruncation/PseudoEvents/triggerRate_avEventPacket_regTrig_V10'
    os.system('mkdir -p %s' % resdir)
    # Do scans of L1 rate                                                                                                                                            
    for rate in range(40, 66, 2):
        for bufflevel in range(1850, 2600, 100):
