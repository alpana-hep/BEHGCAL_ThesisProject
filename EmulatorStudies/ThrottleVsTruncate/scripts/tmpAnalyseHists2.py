...
#    ('triggerRate_avEventPacket_V10_pseudo','triggerRate','Default'),
 ('econdHitScaling_default_V10_pseudo','econdHighBufferLevel','econdHitScaling','Default') ,#'econdHitScaling','Scaled_Occupancy'),
...
x_vals_v0 = set()
dat1['econdHitScaling_default_V10_pseudo'] = dict()

for level in range(1600, 1701, 100):
    for dirname, index_on,  index_on1,label in scans:
        
        files = glob('%s/HGCalBufferModel_%i_buffer_*.root' %(basedir,level)) #files = glob('%s/%s/HGCalBufferModel_*.root' % (basedir, dirname))
        proc_files = glob('%s/HGCalBufferModel_processed_%i_buffer_*.root' %(basedir1,level)) #proc_files = glob('%s/%s/HGCalBufferModel_processed_*.root' % (basedir, dirname))
        #print(files)
    dat[dirname] = dict()
    #dat1[dirname] = dict()
    ...
    for fin in files: ## looping over input files and read histograms
        ...
        f.Close()
        #print(x_vals, x_vals1)
    x_vals_v0.add(x_vals1)
    #dat1[dirname][x_vals_v0] = dict()
    #dat1['econdHitScaling_default_V10_pseudo'][x_vals1]= dat[dirname]
    print(x_vals, x_vals1)
    ...
    for fin in proc_files:
        ...
        f.Close()
                #dat1[dirname][x_vals1]= dat[dirname][x_val]
    for dirname in dat:
    ...
    print('level', level)
    dat1['econdHitScaling_default_V10_pseudo'][level]= (dat['econdHitScaling_default_V10_pseudo'])
...
def GenericOverlayTH1(dat1,
    ...
    for packed in inputs:
        ...
        x_vals = overlay_points
        if len(overlay_points) == 0:
            x_vals = sorted(dat1[dirname].keys()) ## has to be scaling factor
        ...
        if graphbin is not None:
            ...
            for x_val in x_vals:
                ...
                if graphbin == 'mean':
                    y_vals.append(dat1[dirname][1600][x_val][histname].GetMean())
                else:
                    y_vals.append(dat1[dirname][1600][x_val][histname].GetBinContent(graphbin))
                    y_vals_err.append(dat1[dirname][1600][x_val][histname].GetBinError(graphbin))
                    #y_vals_err = [] ## remove it later

                print('x_val',x_val)
                if graphbin_x is not None:
                    x_vals_roc.append(dat1[dirname][1600][x_val][graphbin_x[0]].GetBinContent(graphbin_x[1]))  
                    x_vals_err.append(dat1[dirname][1600][x_val][graphbin_x[0]].GetBinError(graphbin_x[1]))
            if graphbin_x is not None:
                x_vals = x_vals_roc
            if aggr is not None:
            ...
            for x_val in x_vals:
                if histname not in dat1[dirname][1600][x_val]:
                    print('Warning, %s not in %s/%s' % (histname, dirname, x_val))
                drawlist.append(dat1[dirname][1600][x_val][histname].Clone())
                ...
                drawlist[-1]._leg_text = '%s_%i' %(leg_label,x_val/1000)
                if normalize:
    ...
    if isinstance(drawlist[0], ROOT.TGraph):
        ...
        for i in range(0,len(x_vals1)):
            latex = ROOT.TLatex(drawlist[0].GetX()[i], drawlist[0].GetY()[i],"%i" %x_vals1[i])
            latex.SetTextSize(0.03); latex.SetTextColor(colors[6])
    ...
    for h in drawlist:
        ...
        h.Draw(style+'SAME')
        legend.AddEntry(h, h._leg_text, 'P' if 'P' in style else 'L')
    if ratio:
        pads[1].cd()
        axis_ratio.Draw()
        for h in rlist:
...
#print(dat[econdbufferlevel_default_V10])1000,1100,1200,1300,1400,1500,1600,1700,1800]
GenericOverlayTH1(dat, inputs=[
    ('econdHitScaling_default_V10_pseudo',[], 'Scale Factor (F): %s'),
