...
#    ('triggerRate_avEventPacket_V10_pseudo','triggerRate','Default'),
 ('econdHitScaling_default_V10_pseudo','econdHighBufferLevel','Default') ,#'econdHitScaling','Scaled_Occupancy'),
...
for dirname, index_on, label in scans:
    files = glob('%s/HGCalBufferModel_1700_buffer_*.root' %basedir) #files = glob('%s/%s/HGCalBufferModel_*.root' % (basedir, dirname))
    proc_files = glob('%s/HGCalBufferModel_processed_1700_buffer_*.root' %basedir1) #proc_files = glob('%s/%s/HGCalBufferModel_processed_*.root' % (basedir, dirname))
    #print(files)
    dat[dirname] = dict()
...
def GenericOverlayTH1(dat,
    ...
    for packed in inputs:
        ...
        if graphbin is not None:
            ...
            for x_val in x_vals:
                ...
                if graphbin == 'mean':
                    y_vals.append(dat[dirname][x_val][histname].GetMean())
                else:
                    y_vals.append(dat[dirname][x_val][histname].GetBinContent(graphbin))
                    y_vals_err.append(dat[dirname][x_val][histname].GetBinError(graphbin))
                    #y_vals_err = [] ## remove it later

                print('x_val',x_val)
                if graphbin_x is not None:
                    x_vals_roc.append(dat[dirname][x_val][graphbin_x[0]].GetBinContent(graphbin_x[1]))  
                    x_vals_err.append(dat[dirname][x_val][graphbin_x[0]].GetBinError(graphbin_x[1]))
            if graphbin_x is not None:
                x_vals = x_vals_roc
            if aggr is not None:
    ...
    if isinstance(drawlist[0], ROOT.TGraph):
        ...
        for i in range(0,len(x_vals1)):
            latex = ROOT.TLatex(drawlist[0].GetX()[i], drawlist[0].GetY()[i]+0.015*drawlist[0].GetY()[i],"%i" %x_vals1[i])
