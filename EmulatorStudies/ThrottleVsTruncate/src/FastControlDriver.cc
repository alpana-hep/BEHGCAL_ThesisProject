...
FastControlDriver::FastControlDriver() :
  ...
  numberOfLiveThrottles_Econd_(0),
  numberOfLiveThrottles_Hgcroc_(0),
  numberOfLiveThrottles_BE_(0),
  numberOfL1As_(0),
  numberOfL1As_wThrottle_(0),
  numberOfL1As_sent_(0),
...
void FastControlDriver::subProcessBeRequests() {
  ...
  for(unsigned i(0);i<rxRequestsPtrVector_.size();i++) {
    ...
    // Read in requests
    if(commands_.collisionBc() && req.hgcrocBeThrottle()) { numberOfLiveThrottles_Hgcroc_++; LiveThrottles_Hgcroc=true;}
    if(commands_.collisionBc() && req.econdAsicThrottle()) {numberOfLiveThrottles_Econd_++; LiveThrottles_Econd=true;}
    if(commands_.collisionBc() && req.econdBeThrottle()) { numberOfLiveThrottles_BE_++; LiveThrottles_BE=true;}
  ...
  // h1_deadTimewithTime_->Fill(fastControlDriverPtr_->numberOfLiveThrottles_/fastControlDriverPtr_->numberOfLiveBx_);
...
void FastControlDriver::subProcessTcdsCommands() {
  ...
  // Find out how many L1As were stopped due to throttle
  if(tcdsCommands_.collisionBc()) {
    if( trigger()) {
      numberOfL1As_++;
      if(tcdsCommands_.throttle()) numberOfL1As_wThrottle_++;
  ...
  if(calibrationDelayQueue_.front() && !tcdsCommands_.throttle()) {
    tcdsCommands_.setL1aTo(true);
  }

  // // Find how many L1As were issued in total
  // if(tcdsCommands_.collisionBc() && trigger()) {
  //   numberOfL1As_++;
  // }
...
void FastControlDriver::subProcessBeCommands() {
  ...
  //bypassing the analyse flag
  fastControlDriverAnalyserPtr_->analyse();
...
void FastControlDriver::print() const {
  ...
  for(unsigned i(0);i<FastControlCommands::Range;i++) {
    std::cout << "  " << i<<"\t"<<FastControlCommands::commandName((FastControlCommands::Command)i) << " = " << std::setw(10) << commandCount_[i] << std::endl;
  }
  ...
  std::cout << "  Overall throttle    = " << std::setw(10) << commandCount_[32] << std::endl;//"\t"<< "FC Command = "<<FastControlCommands::commandName((FastControlCommands::Command)32)<<std::endl;

  std::cout << std::endl << " Counts of fast control requests" << std::endl;
  for(unsigned i(0);i<FastControlRequests::Range;i++) {
    std::cout << "  " << i<<" "<<FastControlRequests::requestName((FastControlRequests::Request)i) << "   = "<< std::setw(10) << requestCount_[i] << std::endl;
  }
  std::cout << "  Overall throttle    = "<< std::setw(10) << requestCount_[16] << std::endl;//"\t"<< "FC command request= " <<FastControlRequests::requestName((FastControlRequests::Request)16)<<std::endl;
  ...
  std::cout << std::endl
	    << "Number of total L1As should have sent = " << numberOfL1As_ <<std::endl
	    << "Number of total L1As stopped due to throttling = "<<numberOfL1As_wThrottle_<<std::endl
	    << "Number of total L1As sent = "<<numberOfL1As_sent_<<std::endl;
  std::cout << std::endl
