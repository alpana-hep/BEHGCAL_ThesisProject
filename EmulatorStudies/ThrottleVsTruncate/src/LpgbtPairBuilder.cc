...
void LpgbtPairBuilder::subProcessInput() {
  ...
  if(txRamWordPtr_==0 || !txRamWordPtr_->full()) {
    ...
    //    if(outputEcond_>econdAsicDefinitionVector_.size()) outputEcond_ = econdAsicDefinitionVector_.size()-1;
	...
	if(!l1aCountersQueue_.empty()) {
	  ...
	  if(boolHeaderVector_[outputEcond_].first==0) {
		...
		outputEcond_=(outputEcond_+1)%econdAsicDefinitionVector_.size(); //added as a fix to this error
		//outputEcond_++;
	      }
	    }
	  } else {
