...
void EcondAsic::setArgumentsTo(const Arguments &a) {
  ...
  configuration_.setfileReadModeTo(a.config().at("fileReadMode").get<int>());
  ...
  configuration_.setHitlayerArgTo(a.econdHitlayerArg());
...
void EcondAsic::subProcessL1a() {
  ...
  if(hgcrocModel_.l1a(counters_)) {
    ...
    if(rxHgcrocDataPtr_!=0) {
      ...
      if(configuration_.ScintiHitScaling()==0 && configuration_.hitScaling() )
	{
	  if(debugPrint_)
	  	    std::cout << " Numbers of pre-scaled hits = " << nHits[0] << ", "
	  	      << nHits[1] << ", " << nHits[2] << std::endl;
	  if(configuration_.hitAdditiveArg()>0)// && definition_.layer()== configuration_.hitlayerArg())
	    {
	      switch (int(configuration_. hitAdditiveArg())) {
	      case 1:
		//testing the extreme high n20 cases 
		//==================================
		//		nHits[0]+=nHits[1];
		//		if(nHits[0]>(definition_.numberOfChannels()-nHits[1]-nHits[2])) nHits[0] = definition_.numberOfChannels()-nHits[1]-nHits[2]; 
		// if(definition_.type()==1 && definition_.shape()=='F')
		//   {  
		//==================================//
		nHits[0]+=(configuration_.hitScaleFactor()*0.1*definition_.numberOfChannels());
		if(unsigned((nHits[0]+nHits[1]+nHits[2]))>(definition_.numberOfChannels()))
		  nHits[0] = definition_.numberOfChannels()-nHits[1]-nHits[2];

		//std::cout<<"after scaling-Alpana-case1: "<<definition_.layer()<<"\t"<<definition_.u()<<"\t"<<definition_.v()<<"\t"<<"\t"<<nHits[0]<<"\t"<<nHits[1]<<"\t"<<nHits[2]<<"\t"<<definition_.numberOfChannels()<<"\t"<<std::endl;	      
		break;
	      case 2:
		nHits[1]+=(configuration_.hitScaleFactor()*0.1*definition_.numberOfChannels());
		if(unsigned((nHits[0]+nHits[1]+nHits[2]))>(definition_.numberOfChannels()))
                  nHits[1] = definition_.numberOfChannels()-nHits[0]-nHits[2];

		//std::cout<<"after sclaing-Alpana-case2: "<<definition_.layer()<<"\t"<<definition_.u()<<"\t"<<definition_.v()<<"\t"<<nHits[0]<<"\t"<<nHits[1]<<"\t"<<nHits[2]<<"\t"<<definition_.numberOfChannels()<<"\t"<<std::endl;

		break;
	      case 3:
		nHits[2]+=(configuration_.hitScaleFactor()*0.1*definition_.numberOfChannels());
		if(unsigned((nHits[0]+nHits[1]+nHits[2]))>(definition_.numberOfChannels()))
                  nHits[2] = definition_.numberOfChannels()-nHits[1]-nHits[0];

		//std::cout<<"after sclaing-Alpana-case3: "<<definition_.layer()<<"\t"<<definition_.u()<<"\t"<<definition_.v()<<"\t"<<nHits[0]<<"\t"<<nHits[1]<<"\t"<<nHits[2]<<"\t"<<definition_.numberOfChannels()<<"\t"<<std::endl;
		break;
	      case 4:
		for(unsigned i(0);i<3;i++) {
		  int nHitss = nHits[i];
		  //std::cout<<"Alpana-  before scaling-case4: "<<definition_.layer()<<"\t"<<definition_.u()<<"\t"<<definition_.v()<<"\t"<<nHits[0]<<"\t"<<nHits[1]<<"\t"<<nHits[2]<<"\t"<<definition_.numberOfChannels()<<"\t"<<std::endl;
		  nHits[i]+=(configuration_.hitScaleFactor()*0.1*definition_.numberOfChannels());
		  if(unsigned((nHits[0]+nHits[1]+nHits[2]))>(definition_.numberOfChannels()))
		    nHits[i] = nHitss;
		}
		//std::cout<<"Alpana- after scaling case4: "<<definition_.layer()<<"\t"<<definition_.u()<<"\t"<<definition_.v()<<"\t"<<nHits[0]<<"\t"<<nHits[1]<<"\t"<<nHits[2]<<"\t"<<definition_.numberOfChannels()<<"\t"<<std::endl;
		...
		if(unsigned((nHits[0]+nHits[1]+nHits[2]))>(definition_.numberOfChannels()))
                  nHits[i] = nHitss;//definition_.numberOfChannels()-nHits[1]-nHits[2];
		}
		//std::cout<<"after scaling-Alpana: case-5 "<<definition_.layer()<<"\t"<<definition_.u()<<"\t"<<definition_.v()<<"\t"<<nHits[0]<<"\t"<<nHits[1]<<"\t"<<nHits[2]<<"\t"<<definition_.numberOfChannels()<<"\t"<<std::endl;
		break;
	      case 51:
		...
		//std::cout<<"after scaling-Alpana: case 51 "<<definition_.layer()<<"\t"<<definition_.u()<<"\t"<<definition_.v()<<"\t"<<nHits[0]<<"\t"<<nHits[1]<<"\t"<<nHits[2]<<"\t"<<definition_.numberOfChannels()<<"\t"<<std::endl;
		...
		break;
		  ...
		  //std::cout<<"after sclaing-Alpana: "<<definition_.layer()<<"\t"<<definition_.u()<<"\t"<<definition_.v()<<nHits[0]<<"\t"<<nHits[1]<<"\t"<<nHits[2]<<"\t"<<definition_.numberOfChannels()<<"\t"<<std::endl;
        ...
        else if(configuration_.ScintiHitScaling()==1 && configuration_.hitScaling() )
        {
	  if(debugPrint_)
	  ...
	  if(configuration_. hitAdditiveArg()>0)
            {
      	      switch (int(configuration_. hitAdditiveArg())) {
      	      case 1:
		nHits[0]*=configuration_.hitScaleFactor();
		if(unsigned((nHits[0]+nHits[1]+nHits[2]))>(definition_.numberOfChannels()))
                  nHits[0] = definition_.numberOfChannels()-nHits[1]-nHits[2];

		//std::cout<<"after scaling-Alpana: "<<definition_.layer()<<"\t"<<definition_.u()<<"\t"<<definition_.v()<<nHits[0]<<"\t"<<nHits[1]<<"\t"<<nHits[2]<<"\t"<<definition_.numberOfChannels()<<"\t"<<std::endl;
		break;
	      case 2: 
		nHits[1]*=configuration_.hitScaleFactor();
		if(unsigned((nHits[0]+nHits[1]+nHits[2]))>(definition_.numberOfChannels()))
                  nHits[1] = definition_.numberOfChannels()-nHits[0]-nHits[2];
		...
		break;
	      case 3:
		nHits[2]*=configuration_.hitScaleFactor();
		if(unsigned((nHits[0]+nHits[1]+nHits[2]))>(definition_.numberOfChannels()))
                  nHits[2] = definition_.numberOfChannels()-nHits[1]-nHits[0];
      ...
      else if (configuration_.ScintiHitScaling()==2 && configuration_.hitScaling() )
	{
	  if(debugPrint_)
	    std::cout << " Numbers of pre-scaled hits = " << nHits[0] << ", "
                      << nHits[1] << ", " << nHits[2] << std::endl;
		...
		break;
	      case 3:
                nHits[2]+=(configuration_.hitScaleFactor()*0.1*definition_.numberOfChannels());
		if(unsigned((nHits[0]+nHits[1]+nHits[2]))>(definition_.numberOfChannels()))
                  nHits[2] = definition_.numberOfChannels()-nHits[0]-nHits[1];
      ...
      else if (configuration_.ScintiHitScaling()==3 && configuration_.hitScaling() )
        {
          if(debugPrint_)
	    std::cout << " Numbers of pre-scaled hits = " << nHits[0] << ", "
                      << nHits[1] << ", " << nHits[2] << std::endl;
      ...
      // if(configuration_.hitScaling()) {
      // 	if(debugPrint_) std::cout << " Numbers of pre-scaled hits = " << nHits[0] << ", "
      // 				  << nHits[1] << ", " << nHits[2] << std::endl;
    ...
    //    std::cout<<"passed"<<"\t"<<nHits[0]<<"\t"<<nHits[1]<<"\t"<<nHits[2]<<"\t"<<definition_.numberOfChannels()<<"\t"<<definition_.layer()<<"\t"<<definition_.u()<<"\t"<<definition_.v()<<"\t"<<std::endl;

    // Check immediately
    //    std::cout<<"Test alpana "<<configuration_.readMode()<<std::endl;
    if(configuration_.readMode()!=4){
    if(!PacketChecker::checkEcondAsicPacket32(definition_,counters_,packetVector_,debugPrint_)) {
      ...
      printBxTick();
      std::cout<<"packet check failed "<<"\t"<<nHits[0]<<"\t"<<nHits[1]<<"\t"<<nHits[2]<<"\t"<<definition_.numberOfChannels()<<"\t"<<definition_.layer()<<"\t"<<definition_.u()<<"\t"<<definition_.v()<<"\t"<<std::endl;
      assert(false);
    }
    }
...
void EcondAsic::subProcessElinks() {
  ...
  if(maxBufferSize_<bq) {
    maxBufferSize_=bq;
    BX_maxBufferSize_ = Clocks::clocks().bxNumber();
    if(debugPrint_) {
      std::cout << "  New max buffer size = " << maxBufferSize_ << " total number of BX  "<<BX_maxBufferSize_<<std::endl;
    }
  }
...
unsigned long EcondAsic::BX_maxBufferSize() const {  
  return BX_maxBufferSize_;
} //added by Alpana
