...
void HGCHitProvider::processEvent(unsigned long ievt) {
  ...
  for (unsigned t_idx = 0; t_idx < tN; ++t_idx) {
    ...
    if (tmod == mods_[my_idx]) {
      ...
      // if(tmod.layer==1 && tmod.u==2 && tmod.v==2 && (orig_hits_[my_idx].nhits20 >300 || orig_hits_[my_idx].nhits10>300 || orig_hits_[my_idx].nhits30>300))
      // 	std::cout<<"HGCHitProvider "<<tmod.layer<<"\t"<<tmod.u<<"\t"<<tmod.v<<"\t"<< orig_hits_[my_idx].nhits10<<"\t"<< orig_hits_[my_idx].nhits20<<"\t"<< orig_hits_[my_idx].nhits30<<std::endl;
  ...
  for (unsigned idx = 0; idx < mods_.size(); ++idx) {
    ...
    //    if(tmod.layer==1 && tmod.u==2 && tmod.v==2)
    // if(trans_hits_[idx].nhits10>300 || trans_hits_[idx].nhits20>300 || trans_hits_[idx].nhits30>300)
    //   std::cout<<"Check in HGCHIT "<<trans_hits_[idx].nhits10<<"\t"<<trans_hits_[idx].nhits20<<"\t"<<trans_hits_[idx].nhits30<<"\t"<<std::endl;
...
bool HGCHitProvider::nextEvent() {
  if (read_mode_ == 3 || read_mode_ == 4) {
    if (average_hits_.size() == 0) {
     ...
     // if (i % 1000 == 0) { // || trans_hits_[j].nhits10>300 || trans_hits_[j].nhits20>300 || trans_hits_[j].nhits30>300 ) {
     //      std::cout << "[HGCHitProvider::nextEvent] Processed " << i << "/" << entries << " events for averages  " <<trans_hits_[j].nhits10<<"\t"<<trans_hits_[j].nhits20<<"\t"<<trans_hits_[j].nhits30<< std::endl;
     // 	}
	//if(trans_[j].layer==1 && trans_[j].u==2 && trans_[j].v==2)
	// if(
	// std::cout<<"Check in HGCHIT 2nd "<<trans_hits_[j].nhits10<<"\t"<<trans_hits_[j].nhits20<<"\t"<<trans_hits_[j].nhits30<<"\t"<<std::endl;
      ...
      if (read_mode_ == 4) {
        for (unsigned j = 0; j < average_hits_.size(); ++j) {
          ...
          average_hits_[j].nhitsBXM1 = 0x0;       
	  //	  std::cout << "after read mode 4 " << j << "/" << entries << " events for averages  " <<trans_hits_[j].nhits10<<"\t"<<trans_hits_[j].nhits20<<"\t"<<trans_hits_[j].nhits30<< std::endl;
        }
      }
